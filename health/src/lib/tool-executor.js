  ...
  function analyze(parsedMedicines) {
    ...
    const result = {
      ...
      recommendation: fatigueRisk.length > 0
        ? ⚠ ${fatigueRisk.length} medication(s) may contribute to fatigue: ${fatigueRisk.map(m => m.name).join(', ')}. Consult your doctor.
...
async function createFitnessPlan(supabase, userId, planData) {
  try {
    ...
    if (existingPlan && existingPlan.id) {
      ...
      console.log('⚠ Plan already exists. Replacing with new one.');
...
async function createMealPlan(supabase, userId, mealData) {
  try {
    ...
    if (existingPlan && existingPlan.id) {
      ...
      console.log('⚠ Meal plan already exists. Replacing with new one.');
